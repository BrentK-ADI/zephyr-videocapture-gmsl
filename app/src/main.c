...
#if !DT_PROP(DT_CHOSEN(camera_sensor), zephyr_deferred_init)
#error The chosen camera_sensor must have zephyr,deferred-init enabled.
#endif
...
int main(void)
{
	...
	/* Initialize GMSL before anything else */
	err = gmsl_init();
	if (err) {
		LOG_ERR("Failed to initialize GMSL");
		return 0;
	}
	...
	if (!device_is_ready(camera_dev)) {
		LOG_ERR("%s device is not ready", camera_dev->name);
		return 0;
	}

	/* CSI Interface deferred until Camera is running */
	csi_dev = DEVICE_DT_GET(DT_NODELABEL(csi_interface));
	device_init(csi_dev);
	if (!device_is_ready(csi_dev)) {
		LOG_ERR("%s device is not ready", csi_dev->name);
		return 0;
	}
